...
type AliPayTradePay struct {
	...
	SellerId             string             `json:"seller_id,omitempty"`             // 可选 如果该值为空，则默认为商户签约账号对应的支付宝用户ID
	TotalAmount          string             `json:"total_amount,omitempty"`          // 可选 订单总金额，单位为元，精确到小数点后两位，取值范围[0.01,100000000]。 如果同时传入【可打折金额】和【不可打折金额】，该参数可以不用传入； 如果同时传入了【可打折金额】，【不可打折金额】，【订单总金额】三者，则必须满足如下条件：【订单总金额】=【可打折金额】+【不可打折金额】
	...
	UnDiscountableAmount string             `json:"undiscountable_amount,omitempty"` // 可选 不参与优惠计算的金额，单位为元，精确到小数点后两位，取值范围[0.01,100000000]。如果该值未传入，但传入了【订单总金额】和【可打折金额】，则该值默认为【订单总金额】-【可打折金额】
	...
	StoreId              string             `json:"store_id,omitempty"`              // 可选 商户门店编号
	TerminalId           string             `json:"terminal_id,omitempty"`           // 可选 商户机具终端编号
	AliPayStoreId        string             `json:"alipay_store_id,omitempty"`       // 可选 支付宝的店铺编号
	...
	AuthNo               string             `json:"auth_no,omitempty"`               // 预授权号，预授权转交易请求中传入
}

func (p *AliPayTradePay) APIName() string {
	return "alipay.trade.pay"
}

func (p *AliPayTradePay) Params() map[string]string {
	var m = make(map[string]string)
...
func (p *AliPayTradePay) ExtJSONParamName() string {
	return "biz_content"
}

func (p *AliPayTradePay) ExtJSONParamValue() string {
	return marshal(p)
}
